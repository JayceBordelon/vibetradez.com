...
--
-- Creates the schema (matching internal/store/store.go) and inserts ~1 year
-- of realistic test trades with corresponding EOD summaries, plus 5 demo
-- auto-execution scenarios so the local dashboard / history / trade detail
-- pages have visible badge examples.
--
...
-- ─── Trades + Summaries ────────────────────────────────────────────────────
--
-- ~1 year of trade history (252 weekdays) with 10 picks each, all from
-- Claude. Mix of winners and losers across CALL/PUT, varied risk levels,
-- varied sentiment. The most recent date is fully populated WITHOUT
-- summaries (simulating a "morning picks" pre-EOD state). All other days
-- have full EOD summaries so /history renders rich cumulative-P&L curves.
    ...
    WHILE weekday_count < 252 LOOP
        IF EXTRACT(DOW FROM d) NOT IN (0, 6) THEN
            ...
            FOR pick_idx IN 1..10 LOOP
                ...
                /*
                Rank-derived score: rank 1 gets the highest conviction (8-10),
                tail picks (8-10) get lower scores (3-6). Adds a per-day jitter
                so consecutive days don't flatline at identical numbers.
                */
                score_val := GREATEST(1, LEAST(10, (11 - pick_idx) + (((pick_idx * 13 + weekday_count * 7 + ascii(substr(sym, 1, 1))) % 5) - 2)));
                rationale_val := rationales[1 + ((pick_idx - 1) % 10)];
                ...
                IF NOT is_today THEN
                    ...
                    some double, some flatline.
                    */
                    pnl_pct := -1.0 + (((pick_idx * 23 + weekday_count * 31 + ascii(substr(sym, 1, 1))) % 280) / 100.0);
...
BEGIN
    FOR scenarios IN
        SELECT * FROM (VALUES
            (0,  'paper', 'holding',    1.00, NULL),
            (2,  'paper', 'closed-win', 1.00, 1.45),
            (4,  'paper', 'closed-loss',1.00, 0.62),
            (6,  'live',  'closed-win', 0.98, 1.80),
            (8,  'paper', 'failed',     NULL, NULL)
        ) AS s(offset_days, mode, state, fill_open_pct, fill_close_pct)
    LOOP
        SELECT id, symbol, contract_type, strike_price, expiration, estimated_price, score
        INTO t
        FROM trades

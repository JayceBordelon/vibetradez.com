...
- **Caps are percentages of live equity** (40% per name, 50% options sleeve, 30% per order, 50%/session deployment, -35% drawdown breaker, liquidity floor, settled-cash only). Enforced at the tool layer AND re-validated at the broker entry point. Don't relax them in the prompt, they live in code.

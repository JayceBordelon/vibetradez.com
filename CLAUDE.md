...
- **It's one account, not a multi-user product.** UI and copy treat it as a single book; subscribers watch and get the model-authored recap emails, they don't trade.
...
- **The model trades options only and sizes within code-enforced caps.** Equity buys are disabled (`buy_equity` is removed from `ToolDefinitions` and hard-refused in `dispatchBuyEquity`); legacy stock is liquidated via `sell_equity`, and all new exposure is options. Three buy-side gates run at the **tool layer only** (`guards.go` `Check*`): the settled-cash rule (broker T+1 compliance); `CheckOptionSize` caps a single option position at `MaxSingleOptionFrac` (~10%) of equity; `CheckUnderlyingExposure` caps total exposure to one underlying at `MaxPerUnderlyingFrac` (~25%). Sell validation rejects an oversell or a sell of a position not held; sells/de-risking are never blocked. The broker entry point re-checks only a flat $25k absolute fat-finger ceiling (`exec.MaxPortfolioOrderCostCeiling`), a code-bug backstop. The tool layer is the sole enforcement point — keep the options-only mandate and the sizing caps in sync between the prompt (`internal/portfolio/prompt.md`) and `guards.go`.

...
- **Caps are percentages of live equity** (40% per name, 50% options sleeve, 30% per order, 75%/session deployment, -35% drawdown breaker, liquidity floor, settled-cash only). The percentage cap sheet is enforced at the **tool layer only** (`caps.go` `Check*`). The broker entry point re-checks only a flat $25k absolute fat-finger ceiling (`exec.MaxPortfolioOrderCostCeiling`), NOT the percentages, so the tool layer is the sole enforcement point for the percentage policy. Don't relax them in the prompt, they live in code.

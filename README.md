...
At 9:30:00 sharp, the at-open agent (`internal/execagent`) re-invokes Claude with the saved candidates plus a locked-down tool surface (live Schwab quotes, live option chain restricted to candidate symbols, account funds, web search, and a `place_options_order` tool). For each candidate the model reads the now-live chain, picks the strike + expiration + limit price, and either calls `place_options_order` (which writes the execution row + stamps the contract spec on the trade row + submits the BUY_TO_OPEN LIMIT for a single contract in one shot) or declines with a written reason. The agent buys at most one contract of each pick it believes in and never sizes up; the combined price of the three contracts is not a reason to drop a pick. Skipped candidates land in the database as `executions.status='skipped'` rows with the reason in `error_message`. The agent run is fire-and-forget; the per-minute reconcile cron flips fills throughout the morning. At 09:35 the cancel-dangling cron kills any still-WORKING LIMIT and sends ONE consolidated execution-summary email covering every candidate. The 15:55 cron sells every open position and sends ONE consolidated close-summary email.

## Auto-execution gating

If `TRADING_ENABLED=true`, the 9:30:00 ET cron dispatches the at-open agent against the 3 saved candidates. The model makes a per-candidate buy-one-or-skip decision through tool calls; every hard cap is enforced at the tool layer (`internal/execagent/tools.go`), so a buggy prompt or jailbroken model cannot exceed them. The agent buys exactly one contract of each pick whose setup still holds and does not size up; conviction is expressed by which picks it takes, not by how many contracts of one.

| Constant | Value | Why |
|---|---|---|
| `execagent.MaxContractsPerOrder` | 1 | Every order is exactly one contract; the tool refuses any `quantity` above 1. This is the rule that keeps the day to one contract per pick |
| `execagent.MaxOrdersPerRun` | 3 | One order per rank, mirrors candidate basket size |
| `execagent.MaxToolPremiumPerShare` | $10.00 | Per-share LIMIT sanity cap on a single share price |
| `execagent.MaxDailyExposure` | $3,000.00 | Defensive outer ceiling on cumulative `limit_price × 100 × quantity`. With one contract per pick, three picks, and the $10/share cap the day can never exceed `3 × $10 × 100 = $3,000`, so it never blocks buying one of each |
| `exec.MaxContractPremium` | $10.00 | Broker-facing per-share twin; re-validated in `PlaceBuyToOpenAgent` |
| `exec.MaxOrderCost` | $1,000.00 | Broker-facing per-order total-cost twin (`limit × 100 × 1`); a single contract can't exceed it |
...
Worst-case daily blast radius is at most three single contracts, each bounded by the $10/share per-share cap ($1,000 per contract), so $3,000 in the theoretical extreme; in practice the OTM premiums these picks target keep it far lower. The agent buys one contract of each pick it believes in and skips the rest.
...
- **Auto-execution basket.** Single-pass agent. The picker returns exactly 3 ranked candidates; the at-open agent fires at most one single-contract order per rank against the live Schwab chain. It buys one contract of each pick whose setup still holds and never sizes up; conviction is expressed by which picks it takes, not by how many contracts of one. Gated by a $10/share per-share sanity cap, a $1,000 per-order total-cost cap, a one-contract-per-order cap, and a $3,000 defensive daily ceiling, so the day is at most three single contracts.

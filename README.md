...
At 9:30:00 sharp, the at-open agent (`internal/execagent`) re-invokes Claude with the saved candidates plus a locked-down tool surface (live Schwab quotes, live option chain restricted to candidate symbols, account funds, web search, and a `place_options_order` tool). For each candidate the model reads the now-live chain, picks the strike + expiration + limit price + quantity, and either calls `place_options_order` (which writes the execution row + stamps the contract spec on the trade row + submits the BUY_TO_OPEN LIMIT in one shot) or declines with a written reason. The agent works against a $1,000 total daily exposure budget and is encouraged to buy duplicate contracts on high-conviction picks rather than one of each. Skipped candidates land in the database as `executions.status='skipped'` rows with the reason in `error_message`. The agent run is fire-and-forget; the per-minute reconcile cron flips fills throughout the morning. At 09:35 the cancel-dangling cron kills any still-WORKING LIMIT and sends ONE consolidated execution-summary email covering every candidate. The 15:55 cron sells every open position and sends ONE consolidated close-summary email.

## Auto-execution gating

If `TRADING_ENABLED=true`, the 9:30:00 ET cron dispatches the at-open agent against the 3 saved candidates. The model makes per-candidate decisions through tool calls, including how many contracts to buy per pick; every hard cap is enforced at the tool layer (`internal/execagent/tools.go`), so a buggy prompt or jailbroken model cannot exceed them. The agent is encouraged to size up (buy duplicate contracts) on high-conviction picks to deploy the daily budget, rather than reflexively buying one of each.

| Constant | Value | Why |
|---|---|---|
| `execagent.MaxDailyExposure` | $1,000.00 | Cumulative `limit_price × 100 × quantity` across ALL orders this run. Master money ceiling and worst-case daily blast radius |
...
| quantity | ≥ 1 | Duplicate contracts are how the agent sizes up on conviction; one order per rank carries the quantity |
| `exec.MaxContractPremium` | $10.00 | Broker-facing per-share twin; re-validated in `PlaceBuyToOpenAgent` |
| `exec.MaxOrderCost` | $1,000.00 | Broker-facing per-order total-cost twin (`limit × 100 × qty`); one order can't blow the day budget on its own |
...
Worst-case daily blast radius is the $1,000 daily exposure budget (down from the previous 3 × $1,000 = $3,000 per-contract model). The agent can concentrate that budget on one pick or spread it across the three.
...
- **Auto-execution basket.** Single-pass agent. The picker returns exactly 3 ranked candidates; the at-open agent fires one order per rank against the live Schwab chain, choosing the quantity per order. It runs against a $1,000 total daily exposure budget (`limit_price × 100 × quantity`, summed across orders) and is encouraged to buy duplicate contracts on high-conviction picks to deploy that budget. Gated by a $10/share per-share sanity cap, a $1,000 per-order total-cost cap, and the $1,000 cumulative daily budget — so worst-case daily exposure is $1,000, concentrated or spread across the three picks.

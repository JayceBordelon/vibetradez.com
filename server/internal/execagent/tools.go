...
the model tries to spam the tool.
*/
...
type ExecutionService interface {
	PlaceBuyToOpenAgent(ctx context.Context, t *trades.Trade, strike float64, expiration string, dte int, limitPrice float64) (orderID string, execID int, err error)
...
func ToolDefinitions() []anthropic.ToolUnionParam {
	return []anthropic.ToolUnionParam{
		...
		{OfTool: &anthropic.ToolParam{
			...
			Description: anthropic.String("Returns the available USD cash in the Schwab trading account. Use this to size the basket if you want — but each order is already one contract by hard rule, so this is mostly informational."),
			InputSchema: anthropic.ToolInputSchemaParam{
				Properties: map[string]any{},
				Required:   []string{},
			},
		...
		{OfTool: &anthropic.ToolParam{
			...
			Description: anthropic.String("Submit a BUY_TO_OPEN LIMIT order for exactly one options contract. Fire-and-forget — returns the Schwab order id immediately, fills land via the per-minute reconcile cron. " +
				"Hard rules enforced by the tool: (1) symbol must match one of the 3 candidates, (2) one order per rank, (3) maximum 3 orders per run, (4) limit_price must be in (0, $10.00] per share, (5) expiration must be YYYY-MM-DD and in the future. " +
...
func (d *ToolDispatcher) dispatchPlaceOptionsOrder(ctx context.Context, input json.RawMessage) string {
	...
	*/
	...
	orderID, execID, err := d.exec.PlaceBuyToOpenAgent(ctx, candidate, args.Strike, args.Expiration, dte, args.LimitPrice)
	if err != nil {
	...
	// Success — record state for dedup + seed the per-rank decision so
	// reconciliation against the agent's final JSON can detect drift.

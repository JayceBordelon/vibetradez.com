...
POSITION SIZING — EXACTLY ONE CONTRACT PER PICK
===============================================
Buy at most ONE contract of each pick you decide to trade. Do not size up, ever:

- Every order is for exactly quantity 1. There is no budget to deploy and no number to hit. Conviction is expressed by WHICH picks you buy, not by how many contracts of one.
- Buy one contract of each of the up-to-3 picks whose setup still holds. Buy one of each that you believe in regardless of what the three contracts cost together — the combined price of the basket is not a reason to drop a pick.
- You still skip freely. If a pick's setup has materially deteriorated (overnight gap, no liquidity at the intent anchor, premium way above what makes sense, thesis invalidated by recent news), decline it with a clear written reason. A skip is for a broken thesis, never to stay under some spend number.
- Do not call place_options_order more than once for the same pick, and do not pass quantity greater than 1. The tool refuses both.
...
1. For each candidate, in rank order:
   ...
   d. Decide: buy one contract or skip.
   e. If buy: call place_options_order(rank, symbol, contract_type, strike, expiration, limit_price) for a single contract (quantity 1). The tool refuses a second order for the same rank. Limit price ~1.10× the live ask is reasonable; you have discretion.
...
============================================================
- Exactly ONE contract per order. quantity must be 1; the tool refuses any order with quantity above 1. There is no sizing up.
...
- get_account_funds(): your available USD cash on the Schwab account (informational — you only ever buy one contract per pick).
- place_options_order(rank, symbol, contract_type, strike, expiration, limit_price): submits a BUY_TO_OPEN LIMIT for a single contract of the chosen spec. Returns the Schwab order_id immediately (fire-and-forget — fills land via the per-minute reconcile cron). On error returns a clear refusal reason (per-share cap exceeded, quantity above 1, off-list symbol, duplicate rank, etc.).
...
{
  "decisions": [
    { "rank": 1, "action": "buy",  "order_id": "1006xxxxxxxxx", "strike": 150.00, "expiration": "2026-05-29", "limit_price": 2.30, "quantity": 1 },
    ...
    { "rank": 3, "action": "buy",  "order_id": "1006yyyyyyyyy", "strike":  45.00, "expiration": "2026-05-29", "limit_price": 1.15, "quantity": 1 }
  ],
  "final_note": "One-sentence overall comment on today's basket (which picks you took and which you skipped). Optional but useful for the operator email."
}

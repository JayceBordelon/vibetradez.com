...
func (a *Agent) runConversation(ctx context.Context, prompt string, dispatcher *ToolDispatcher, rec *transcript.Recorder) (string, error) {
	...
	for round := 0; round < maxToolRounds; round++ {
		...
		// Capture the code-execution container id and thread it onto every
		// later request. web_search_20260209 / web_fetch_20260209 use dynamic
		// filtering: the model runs code in Anthropic's sandbox that itself
		// calls web_search / web_fetch ("code execution with tools"). Those
		// nested tool uses live in a container, and once they exist the API
		// REQUIRES container_id on every following request in the turn — omit it
		// and the next call 400s with "container_id is required when there are
		// pending tool uses generated by code execution with tools", which
		// silently zeroed a live session (0 decisions). The id is only ever
		// reported on the top-level container object; keep the latest non-empty
		// one we see (it is stable across a session) and never clear it.
		if id := responseContainerID(msg); id != "" {
			if containerID == "" {
				log.Printf("portfolio: code-execution container engaged (id=%s)", id)
			}
			containerID = id
		}
		...
		var usedServerCodeExec bool // a code-execution sub-tool ran this round
		...
		// Container-threading diagnostic: if a code-execution sub-tool ran this
		// round but no container id surfaced (neither the typed field nor the
		// raw JSON), the next request that echoes these pending code-execution
		// tool uses is the one the API rejects with "container_id is required".
		// Log it loudly so a live session pinpoints exactly where the contract
		// broke instead of dying with a generic session error.
		if usedServerCodeExec && containerID == "" {
			log.Printf("portfolio: WARNING code execution ran in round %d (stop=%s) but no container id surfaced; a follow-up request may be rejected with container_id required", round, msg.StopReason)
		}
...
/*
responseContainerID returns the code-execution container id from a model
response. The web_search_20260209 / web_fetch_20260209 dynamic-filtering path
and the auto-enabled code-execution sandbox run their tools in a container; the
id is only ever reported on the top-level `container` object and must be threaded
back via MessageNewParams.Container on every later request in the turn (see the
capture site in runConversation). Reads the typed field first, then falls back to
the raw response JSON in case the SDK's presence tracking on the nested object
misses it. Returns "" when the response carries no container.
*/
func responseContainerID(msg *anthropic.Message) string {
	if msg.Container.ID != "" {
	...
	if err := json.Unmarshal([]byte(msg.RawJSON()), &shape); err != nil {
		return ""
	}
	return shape.Container.ID
}

...
/*
One-time email ledger. A one-time blast (e.g. a product-update announcement,
and any future one-off) claims a stable key in sent_emails so it fires exactly
once and never re-sends on a later boot, even if its trigger env flag stays set.
*/

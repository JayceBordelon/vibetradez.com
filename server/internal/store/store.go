	...
	*/
	...
	_, err = db.Exec(`
		...
		*/
		...
		/*
		sent_emails: a one-row-per-key ledger of one-time emails (e.g. a
		product-update announcement) that have already gone out. The send path
		claims a key here so a one-time blast fires exactly once and never
		re-sends on a later boot, regardless of how long its trigger env flag
		stays set.
		*/

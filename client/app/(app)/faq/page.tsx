...
const faqs = [
  ...
  },
  {
    ...
    answer: `Picks 4 through 10: partially hypothetical. The top 3 picks always fire as real live orders every weekday at 9:30 ET against my actual Schwab brokerage account, held until 3:55 ET when the close cron exits unconditionally. After that, the greedy fill walks ranks 1 through 10 and stuffs additional contracts into the same morning batch until cumulative exposure approaches $1,000, so picks 4-10 sometimes get a real fill and sometimes don't depending on the day's premiums. Anywhere a real broker fill exists, the dashboard sources realized P&L from broker truth and tags the row with a clearly-labeled execution badge. The remaining picks fall back to a hypothetical "you bought one contract at the morning mark" model, which doesn't account for spreads, slippage, fills that never fill, or your broker's general mood. Treat the modeled P&L like a backtest some guy made on Reddit: directionally interesting, not a guarantee.`,
  },
  {
  ...
  },
  {
    ...
    answer: `Two phases at 9:30 ET. Phase 1: ranks 1, 2, and 3 always fire as real live orders against my actual Schwab brokerage account, with my actual money. There's a per-contract safety cap at $10/share ($1,000 of capital exposure on a single contract) so a malformed quote can't blow up the account. Phase 2: with whatever budget remains under a $1,000 daily-exposure target, the selector walks ranks 1 through 10 and adds more contracts greedily (rank-1 fills first, then rank-2, and so on) until the budget is gone. Duplicates are allowed and merged into one quantity=N order per pick, so the broker sees one batch. At 3:55 ET (12:55 on half-trading days) the close cron unconditionally sells everything still open, with retry-cancel-replace if the first attempt doesn't fill. There's also a kill-switch endpoint I can hit if the whole thing starts looking sideways before close. Receipt emails go to me, not to subscribers, because nobody else needs to know about my fills.`,
  },
  {

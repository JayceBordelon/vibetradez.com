...
function DesktopTradeRow({ dt }: { dt: DashboardTrade }) {
  ...
  return (
    <Fragment>
      <TableRow className={cn("cursor-pointer border-l-2 transition-colors hover:bg-muted/50", row.accentBorder)} aria-expanded={open} onClick={() => setOpen((v) => !v)}>
        <TableCell className="w-10">{open ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}</TableCell>
...
function TradeRowCard({ dt }: { dt: DashboardTrade }) {
  ...
  return (
    <Card className={cn("animate-in fade-in fill-mode-backwards duration-200 border-l-2", row.accentBorder)}>
...
function TradeDetail({ dt, compact = false }: { dt: DashboardTrade; compact?: boolean }) {
  const { trade, summary } = dt;
  const moneyness = calcMoneyness(trade);
    ...
    <div className={cn("space-y-4", compact ? "" : "p-4")}>
      ...
      {summary && (
        <div className="rounded-md bg-card-elevated px-3 py-2 text-sm">
          <span className="font-semibold">EOD Result:</span> Entry <span className="font-mono tabular-nums">{fmtMoney(summary.entry_price)}</span> &rarr; Close{" "}
          <span className="font-mono tabular-nums">{fmtMoney(summary.closing_price)}</span> &middot; Stock <span className="font-mono tabular-nums">{fmtMoney(summary.stock_open)}</span> &rarr;{" "}
          <span className="font-mono tabular-nums">{fmtMoney(summary.stock_close)}</span>
          {summary.notes && <span className="ml-1 text-muted-foreground">- {summary.notes}</span>}
        </div>
      )}
    </div>
  );

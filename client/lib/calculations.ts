...
export function calcMoneyness(trade: Trade) {
	...
	if (s === 0 || k === 0)
		return { pct: 0, label: "ATM", variant: "outline" as const };
	...
	if (Math.abs(pct) < 1)
		return { pct, label: "ATM", variant: "outline" as const };
	if (pct > 0)
		return {
			pct,
...
export function calcBreakeven(trade: Trade): number {
	return trade.contract_type === "CALL"
		? trade.strike_price + trade.estimated_price
		: trade.strike_price - trade.estimated_price;
}
